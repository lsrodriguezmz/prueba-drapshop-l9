          ...
          <div class="flex flex-col">
            ...
            <form method="POST" action="{{ route('tasks.store') }}" class="w-full max-w-sm">
              ...
              <div class="flex items-center border-b border-teal-500 py-2">
                <input type="text" id="name" name="name" placeholder="Crear nueva Tarea" aria-label="Name" class="appearance-none bg-transparent border-none w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none">
              </div>
              <div class="flex items-center border-b border-teal-500 py-2">
                      ...
                      <tr class="whitespace-nowrap">
                        ...
                        <td class="px-6 py-4">
                          <div class="flex space-x-2">
                            <a href="{{ route('tasks.edit', $task->id) }}" class="px-4 py-1 text-sm text-black bg-blue-400 rounded">Editar</a>
                            <form action="{{ route('tasks.destroy', $task->id) }}" method="POST" onsubmit="return confirm('Comfirmar Accion');" class="px-4 py-1 text-sm text-black bg-red-400 rounded">
                              @csrf
                              @method('DELETE')
